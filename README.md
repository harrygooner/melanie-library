...
- `scripts/import-documents.mjs`: nhập tài liệu từ ZIP vào Vercel Blob và bảng `documents`. Chạy `npm run documents:import` để tạo báo cáo khô; chỉ dùng `npm run documents:import -- --execute` sau khi kiểm tra báo cáo. Script đọc `DATABASE_URL`, `BLOB_READ_WRITE_TOKEN` và các biến import tùy chọn từ `.env`/`.env.local`. Bộ nhập chỉ khớp chính xác thư mục nguyên liệu với `app/data/catalog.json`, không đoán biến thể, giữ tên file gốc, bỏ qua định dạng ngoài danh sách và file lớn hơn 15 MB, đồng thời chống trùng khi chạy lại.
